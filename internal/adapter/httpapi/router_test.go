...
func TestLiveHealthContract(t *testing.T) {
	...
	httpapi.NewHandler(nil, nil, nil, nil, nil, nil).ServeHTTP(response, request)
...
func TestSessionRoutesRejectUnsupportedMethods(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			httpapi.NewHandler(&stubSessionService{}, nil, nil, nil, nil, nil).ServeHTTP(response, request)
...
func TestCreateVerificationSessionHTTPContract(t *testing.T) {
	...
	httpapi.NewHandler(service, nil, nil, nil, nil, nil).ServeHTTP(response, request)
...
func TestSubmitPersonalDetailsHTTPContract(t *testing.T) {
	...
	httpapi.NewHandler(&stubSessionService{}, service, nil, nil, nil, nil).ServeHTTP(response, request)
...
func TestPersonalDetailsRequestLoggingExcludesPIIAndCredentials(t *testing.T) {
	...
	handler := httpapi.WithRequestLogging(httpapi.NewHandler(&stubSessionService{}, service, nil, nil, nil, nil), logger)
	request := httptest.NewRequest(
		http.MethodPost,
		"/verification-sessions/"+id.String()+"/personal-details",
...
func TestHTTPAdapterMapsMalformedAndInternalFailuresToSafeEnvelopes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			httpapi.NewHandler(test.service, nil, nil, nil, nil, nil).ServeHTTP(response, request)
...
func TestRequestLoggingRecordsSafeHTTPOutcomeWithoutCredentials(t *testing.T) {
	...
	var logs bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&logs, nil))
	handler := httpapi.WithRequestLogging(httpapi.NewHandler(service, nil, nil, nil, nil, nil), logger)
	request := httptest.NewRequest(http.MethodGet, "/verification-sessions/"+id.String(), nil)
	request.Header.Set("Authorization", "Bearer "+rawToken)
	response := httptest.NewRecorder()
...
func TestRequestLoggingRecordsAuthenticationErrorCode(t *testing.T) {
	...
	var logs bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&logs, nil))
	handler := httpapi.WithRequestLogging(httpapi.NewHandler(&stubSessionService{}, nil, nil, nil, nil, nil), logger)
	request := httptest.NewRequest(http.MethodGet, "/verification-sessions/"+id.String(), nil)
	response := httptest.NewRecorder()
